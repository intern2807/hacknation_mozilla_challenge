...
# Step 2: Build Harbor extension for Safari
echo ""
echo "Step 2: Building Harbor extension for Safari..."
cd "$EXTENSION_DIR"
...
echo "  Built Harbor extension in $EXTENSION_DIR/dist-safari"

# Step 2b: Build Web Agents extension for Safari
WEB_AGENTS_DIR="$PROJECT_ROOT/web-agents-api"
echo ""
echo "Step 2b: Building Web Agents extension for Safari..."
cd "$WEB_AGENTS_DIR"
...
npm run build:safari
echo "  Built Web Agents extension in $WEB_AGENTS_DIR/dist-safari"
...
if [ -d "$RESOURCES_DIR" ]; then
    ...
    echo "  Harbor Extension synced to $RESOURCES_DIR"
else
    echo "Warning: Harbor Extension Resources directory not found at $RESOURCES_DIR"
fi

# Sync Web Agents extension
WEB_AGENTS_DIST="$WEB_AGENTS_DIR/dist-safari"
WEB_AGENTS_RESOURCES_DIR="$SCRIPT_DIR/Harbor/Web Agents Extension/Resources"

if [ -d "$WEB_AGENTS_RESOURCES_DIR" ]; then
    # Clear old resources and sync
    rm -rf "$WEB_AGENTS_RESOURCES_DIR/manifest.json" "$WEB_AGENTS_RESOURCES_DIR/assets" \
           "$WEB_AGENTS_RESOURCES_DIR"/*.js "$WEB_AGENTS_RESOURCES_DIR"/*.html \
           "$WEB_AGENTS_RESOURCES_DIR"/*.css
    
    # Copy all from dist-safari
    cp -r "$WEB_AGENTS_DIST/"* "$WEB_AGENTS_RESOURCES_DIR/"
    
    echo "  Web Agents Extension synced to $WEB_AGENTS_RESOURCES_DIR"
else
    echo "Warning: Web Agents Extension Resources directory not found at $WEB_AGENTS_RESOURCES_DIR"
    echo "  You may need to add the Web Agents Extension target in Xcode."
    echo "  See: $SCRIPT_DIR/add-web-agents-target.md"
fi
